...
<AccordionGroup>
  ...
  <Accordion title="What runtime do I need?">
    ...
    Bun 1.4+ builds with `node:sqlite` can run the CLI, Gateway, and managed node host as an explicit opt-in.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Can I use Bun?">
    ...
    with `node:sqlite` can run the CLI, Gateway, and managed node host as an
